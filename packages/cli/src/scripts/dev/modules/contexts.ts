import type { ChooksCommand, ChooksContextCommand } from 'chooksie/types';
...
import { validateContextCommand } from '../../../lib/validation';
...
function isCommandValid(
  ...
  const validationError = validateContextCommand(command);
  if (validationError) {
...
export async function update(
  ...
  const commandData = await uncachedImport<ChooksContextCommand>(filePath);
